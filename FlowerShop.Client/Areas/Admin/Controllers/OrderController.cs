...
namespace FlowerShop.Client.Areas.Admin.Controllers
{
    ...
    public class OrderController(IBaseService baseService) : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var orders = await _baseService.GetODataAsync<IEnumerable<OrderDTO>>(
                "Odata/Orders?$expand=OrderItems", token);

            var list = (orders ?? new List<OrderDTO>())
        ...
        public async Task<IActionResult> UpdateStatus(Guid id, [FromBody] OrderUpdateDTO dto)
        {
            ...
            var res = await _baseService.PutAsync<OrderDTO>(
                $"Odata/Orders({id})", dto, token);
